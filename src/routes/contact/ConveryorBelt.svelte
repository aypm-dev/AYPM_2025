...
<script lang="ts">
	...
	function circleLoad(node: HTMLDivElement, index: number) {
		...
		const { x, y } = getPointOnRectangle(
			percentage,
			conveyor_belt_w - 4,
			conveyor_belt_h - 4,
			conveyor_belt_rounding
		);
...
<div
	use:conveyorBeltLoad
	class="flex pointer-events-none mx-auto relative shadow-lg border-[3px] border-b-[6px] border-black bg-white"
>
	{#each { length: amount_of_circles } as _, index}
		<div
			use:circleLoad={index}
			class="absolute w-4 aspect-square bg-white rounded-full -translate-x-1/2 -translate-y-1/2"
		/>
	{/each}
</div>
