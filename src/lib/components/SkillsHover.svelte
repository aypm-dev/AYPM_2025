<script lang="ts">
    ...
    function mouseHoverHandler (event: MouseEvent) {
        ...
        }, { delay: 50, duration: 500, fill: "forwards" })
    }
    ...
    function mouseLeaveHandler ( event: MouseEvent ) {
        ...
        background_2.animate({
            "clipPath": `polygon(0 0, ${x_percent}% 0, ${x_percent}% 100%, 0% 100%)`
        }, { delay: 200, duration: 200, fill: "forwards", easing: "cubic-bezier(.55,.01,.45,1.01)" })
    }
</script>

<div class="container mt-[5.5rem] sm:mt-32 mb-20 sm:mb-32">

    <h1 class="text-center text-2xl font-bold italic"> OUR PARTNERS :) </h1>
    <div bind:this={background_container} on:touchmove={touchMoveHandler} on:mousemove={mouseHoverHandler} on:mouseleave={mouseLeaveHandler} class="relative flex w-5/6 sm:w-1/2 h-full mx-auto rounded-xl select-none bg-red-500">
        <div class="absolute grid grid-cols-4 gap-4 w-full p-4 bg-white rounded-xl border-[3px] border-b-[6px] border-eerie-900">
            ...
            <div class="flex w-2/3 aspect-square mx-auto my-auto justify-center items-center">
                <img style="filter: drop-shadow(0 0.35rem 0.2px black) saturate(90%)" src="html-logo.png"loading="lazy" alt="HTML Logo" draggable="false" class="h-full pointer-events-none">
            </div>
            <div class="flex w-2/3 aspect-square mx-auto my-auto justify-center items-center">
